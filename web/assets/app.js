...
import { checkinScreen, scrollThread } from './checkin-view.js';

const TABS = [
  { id: 'checkin',  ic: '💬', label: 'Check-in' },
...
/**
 * `anthropic/claude-3.5-haiku` → `claude-3.5-haiku`. The provider prefix is
 * dropped because the topbar shares a row with the mark and the dashboard link
 * on a 375px phone, and the full slug pushes them onto two lines.
 */
const shortModel = (slug) => (slug ? String(slug).split('/').pop() : 'model');
...
function renderTabs() {
  clear($('#tabbar')).append(...TABS.map((t) =>
    el('button', {
      ...
      class: t.id === 'checkin' && ck.isPending() ? 'has-dot' : null,
...
const firstName = (u) => (u?.name || '').trim().split(/\s+/)[0] || 'there';

function balanceCard(l) {
  const delta = l.week.meaning - l.prevWeek.meaning;
  return el('section', { class: 'card balance-card' },
    // Named, because a ledger addressed to nobody reads like a report.
    el('h3', { class: 'greet' }, `Hey ${firstName(l.user)}, here is your happiness capital`),
...
function todayScreen() {
  ...
  if (state.lastReframe) {
    nodes.push(el('section', { class: 'card' },
      el('h3', {}, `The agent · ${state.lastReframe.engine === 'openai' ? shortModel(state.me?.model) : 'local engine'}`),
...
function render() {
  renderTabs();
  const views = { checkin: checkinScreen, today: todayScreen, ledger: ledgerScreen, patterns: patternsScreen, room: roomScreen };
  clear(screen).append(...(views[state.tab] || todayScreen)());
  if (state.tab === 'room') loadActuations();
  if (state.tab === 'checkin') requestAnimationFrame(scrollThread);
}
...
(async function boot() {
  ...
  // Name the model that is actually answering rather than a brand, because the
  // same key can be pointed at any of them through OpenRouter.
  $('#engine-pill').textContent = me.engine === 'openai' ? shortModel(me.model) : 'local brain';
  $('#engine-pill').title = me.engine === 'openai'
    ? `Reframes generated by ${me.model}`
    : 'No OPENROUTER_API_KEY set — running the built-in reframe engine';

  const { ledger, viewingSelf } = await api('/api/ledger');
  state.ledger = ledger;

  // Opening the app is a conversation before it is a dashboard. If today's
  // check-in is unfinished, that is where the app lands — unless the person
  // deep-linked to a specific tab.
  ck.onCheckinChange(() => { if (state.tab === 'checkin') render(); });
  document.addEventListener('kindred:rerender', render);
  await ck.loadCheckin().catch(() => {});
  if (!location.hash && ck.isPending()) state.tab = 'checkin';
  ...
  subscribe((e) => {
    if (e.type === 'moment' && e.userId === ledger.user.id) {
      api('/api/ledger').then(({ ledger: fresh }) => { state.ledger = fresh; if (state.tab !== 'checkin') render(); }).catch(() => {});
    }
